...
public class GldAuthNode(params AuthNode[] requires)
    : AuthNode(new AuthConvention().Provides(HNUAuthData.BZB_NJW).Requires(), requires)
{
    ...
    {
        return Task.FromResult(client.Get(new Uri(GldUrl), accept: MediaType.Html)
