...
public class AuthDataTable
{
    ...
    public TResult RequireData<TKey, TResult>(TKey key) where TKey : AuthDataKey<TResult> where TResult : notnull
    {
        return _userAuthDataTable[key] is TResult ? (TResult)_userAuthDataTable[key] : throw new InvalidOperationException("Cannot find required data for key " + key + " with type " + typeof(TResult));
    }
